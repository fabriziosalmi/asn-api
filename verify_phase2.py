...
def test_asn_score():
    ...
    try:
        ...
        if r.status_code == 404:
            print(
                "ASN 3333 not found, trying to seed it via scoring... (Not implemented in this script)"
            )
            return
        ...
        if etag:
            ...
            r2 = requests.get(
                f"{API_URL}/asn/{asn}", headers={**HEADERS, "If-None-Match": etag}
            )
...
def test_peer_pressure():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            print(
                f"ASN: {data['asn']}, Avg Upstream Score: {data['avg_upstream_score']}"
            )
